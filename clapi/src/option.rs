...
impl CommandOption{
    ...
    pub fn new(name: &str) -> Self{
        ...
        let args = Arguments::none()
            .also_mut(|a| a.parent = Some(Symbol::Option(name.to_string())));
    ...
    #[inline]
    pub fn args_as<T>(&self) -> Result<Iter<'_, T>>
        where T : FromStr, <T as FromStr>::Err : Display {
...
impl Options{
    ...
    pub fn get(&self, name_or_alias: &str) -> Option<&CommandOption>{
        self.inner.iter().find(|o| {
            o.name == name_or_alias || o.aliases.contains(name_or_alias)
        })
    }
...
mod tests{
    ...
    #[test]
    fn alias_test(){
        let opt = CommandOption::new("name")
            .set_alias("n")
            .set_alias("nm");
    ...
    #[test]
    fn description_test(){
        let opt = CommandOption::new("date")
            .set_description("Sets the date");
    ...
    fn args_test(){
        ...
        let mut opt2 = opt1.clone()
            .set_args(Arguments::new(1..)
                .set_valid_values(&["day", "hour", "minute"]));
